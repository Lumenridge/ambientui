...
5. **Nothing inside an ambient surface is opaque, and the veil always sits
   ...
   and `screen` (full presence, full frost: a full-screen surface, where the
   field carries a whole window but must still sit under the translucency).
   A place, not two knobs, because every wrong combination has been built at
   least once — full presence under the thin veil leaves the shader raw,
   which is the ground's recipe applied where it does not belong. A pane
...
| 2026-08-23 | **The veil reaches every chrome area, and the composer collects its own controls.** Rule 5 said the veil sits over the field, but each bar that draws its own backdrop has to carry the veil itself — the header, the rail and the composer bar each sampled the field and showed the shader raw through it until they wore the same frost. `.ambient-clear` (blur, no fill) was deleted rather than left lying around: it had exactly one user, and once that user needed a veil, a fill-less recipe was only ever going to be picked wrongly again. The `+` moved from the context row into the composer beside Send — it acts on the turn you are about to send, so it belongs where you are typing — which also made the Composer the single owner of attaching, having already been the single owner of attachments and paste. Staged attachments page with arrows when they overflow | The fill-less recipe is the lesson worth keeping: a variant that exists to omit something will be reached for by someone who does not know what the omission costs. Deleting it was cheaper than documenting when not to use it |
