...
| 2026-08-23 | **A bar's edge becomes a falloff, not a line** — `.ambient-blur-top` / `.ambient-blur-bottom` in theme.css, replacing the hard borders on history's header and composer. A border says "two panes stacked"; a progressive blur says "one surface, and the content continues underneath", which is what a scrolling transcript under a bar actually is. Three layers rather than one, because a single masked blur has a visible seam where its mask ends — each layer blurs more and is revealed over a shorter band, and the band reaches past the bar into the scroll area where the fade needs the room. Also: the readable-column cap moved from the shared transcript onto the surface that needs it. History is full-screen so its column is capped and its scroller reaches the window edge; the panel, dock and spotlight are already narrower than any cap, and applying one there just inset their content from their own edges | The cap was the second time in this session that a constraint got attached to the wrong owner — the first was the composer's `mark`. The question to ask of any prop like this is "whose problem is this?", and the answer is almost never "everyone's" |
