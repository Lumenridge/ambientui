/**
 * THE CONFIGURATION SPACE — every dimension this design system can vary,
 * and the compiler that turns a choice into CSS.
 *
 * DATA AND ONE PURE FUNCTION. No React, no imports, nothing to mount. That
 * is what makes it the shippable form of the argument: a consumer can take
 * this file, call `compileFoundationCss(config)`, and render the result
 * into a <style> tag, at build time or at runtime, with or without the
 * provider — which is one way to use this, not the only one.
 *
 * It was extracted from foundation-context.tsx, where these ~640 lines sat
 * behind a "use client" directive and a React dependency they never needed.
 */
...
/** Bring a stored config forward across shape changes. */
export function migrate(raw: Record<string, unknown>): Partial<FoundationConfig> {
...
/** Where a saved config is persisted. */
export const STORAGE_KEY = "ambientui-foundation"
